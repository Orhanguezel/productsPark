...
function genOrderNumber(prefix = 'ORD') { return `${prefix}${Date.now()}`; }
...
function mapOrder(o: OrderRow) {
  return {
    ...
    total_price: Number(o.total), // RTK'de toNumber uygulanıyor ama biz garanti ediyoruz
    currency: 'TRY',
...
function calcDiscount({ discount_type, discount_value, subtotal, max_discount }: {
  discount_type: DiscountType; discount_value: string; subtotal: string; max_discount?: string | null;
}) {
...
async function redeemCouponAtomic(client: TxOrDb, couponId: string) {
  await client.update(couponsTable)
...
export const getOrder: RouteHandler = async (req, reply) => {
  ...
  try {
    ...
    // normalize + raw birleşik dönüyoruz (RTK sadece ihtiyaç duyduklarını alır)
...
export const checkoutFromCart: RouteHandler = async (req, reply) => {
  try {
    ...
    const normalizedItems = cartRows.map((row) => {
      ...
      return { product_id: row.product_id, product_name: productName, quantity: row.quantity, price: priceStr, total: totalStr, options: row.options ?? null };
    });
    if (missing.length) {
      return reply.code(400).send({ error: { message: 'pricing_required', details: { missing_product_ids: missing } } });
    }
...
export const updateOrder: RouteHandler = async (req, reply) => {
  ...
  try {
    ...
    const [ord] = await db.select({ id: orders.id, user_id: orders.user_id }).from(orders)
      .where(and(eq(orders.id, id), eq(orders.user_id, userId))).limit(1);
    if (!ord) return reply.code(404).send({ error: { message: 'not_found' } });

    await db.update(orders).set({ ...patch, updated_at: now() }).where(eq(orders.id, id));
...
export const updateOrderItem: RouteHandler = async (req, reply) => {
  ...
  try {
    ...
    const [item] = await db.select({ order_id: order_items.order_id }).from(order_items)
      .where(eq(order_items.id, id)).limit(1);
    if (!item) return reply.code(404).send({ error: { message: 'not_found' } });

    const [ord] = await db.select({ id: orders.id, user_id: orders.user_id }).from(orders)
      .where(eq(orders.id, item.order_id)).limit(1);
    if (!ord || ord.user_id !== userId) return reply.code(403).send({ error: { message: 'forbidden' } });

    await db.update(order_items).set({ ...patch, updated_at: now() }).where(eq(order_items.id, id));
