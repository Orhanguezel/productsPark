...
import { topbarSettings, type TopbarRow } from "./schema";
import { coupons, type CouponRow } from "@/modules/coupons/schema";
import {
...
type RowWithCoupon = {
  top: TopbarRow;
  coup: Pick<CouponRow, "code" | "title" | "content_html"> | null;
};
...
function mapRow(r: RowWithCoupon) {
  ...
  return {
    ...
    // Kupon ilişkisi
    coupon_id: tb.coupon_id ?? null,
    coupon_code: c?.code ?? null,
    coupon_title: c?.title ?? null,
    coupon_content_html: c?.content_html ?? null,

    created_at: tb.created_at
      ? new Date(tb.created_at as unknown as Date).toISOString()
      : undefined,
    updated_at: tb.updated_at
      ? new Date(tb.updated_at as unknown as Date).toISOString()
      : undefined,
  };
}
...
export const listTopbarSettings: RouteHandler<{
  Querystring: TopbarPublicListQuery;
}> = async (req, reply) => {
  const q = topbarPublicListQuerySchema.parse(
    req.query ?? {},
  ) as TopbarPublicListQuery;
  ...
  let qb = db
    .select({
      ...
      coup: {
        code: coupons.code,
        title: coupons.title,
        content_html: coupons.content_html,
      },
    })
    .from(topbarSettings)
...
export const getTopbarSettingById: RouteHandler<{
  Params: { id: string };
}> = async (req, reply) => {
  const { id } = req.params;
  ...
  return reply.send(mapRow(row as RowWithCoupon));
};
