// ===================================================================
// FILE: src/modules/payments/admin.payments.controller.ts
// FINAL — Payments Admin
// Changes:
// - capture/refund/void now run in db.transaction (atomic with events)
...
const listPaymentsAdminQuery = z
  .object({
    ...
    sort: z
      .enum(['created_at', 'updated_at', 'amount_captured', 'amount_authorized', 'status'])
      .optional(),
...
const voidPaymentBody = z.object({ reason: z.string().optional() }).partial();
    ...
    (
      await (whereSql
        ? db
            ...
            .from(payments)
            .where(whereSql)
        : db.select({ total: sql<number>`COUNT(*)`.as('total') }).from(payments))
    )[0]?.total ?? 0;
...
export async function capturePaymentAdminHandler(
  ...
  const out = await db.transaction(async (tx) => {
    const [row] = await tx.select().from(payments).where(eq(payments.id, req.params.id)).limit(1);
    if (!row) return null;

    const currentCaptured = Number(row.amountCaptured);
    const maxCapture = Number(row.amountAuthorized) - currentCaptured;
    const amount =
      parsed.data.amount != null ? Math.min(parsed.data.amount, maxCapture) : maxCapture;
  ...
  return reply.send(mapPaymentAdmin(out));
}
...
export async function refundPaymentAdminHandler(
  ...
  const out = await db.transaction(async (tx) => {
    ...
    const amount =
      parsed.data.amount != null ? Math.min(parsed.data.amount, refundable) : refundable;
...
export async function voidPaymentAdminHandler(
  ...
  const out = await db.transaction(async (tx) => {
    ...
    await tx.insert(paymentEvents).values({
      ...
      paymentId: req.params.id,
      eventType: 'void',
      message: `Voided${parsed.data.reason ? ` | reason: ${parsed.data.reason}` : ''}`,
      raw: toJsonString({ reason: parsed.data.reason ?? null }),
    });
