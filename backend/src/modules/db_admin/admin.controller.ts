...
async function exportDbAsJsonBuffer(): Promise<Buffer> {
  const cfg = DB();
  const p = pool();
  const conn = await p.getConnection();
  ...
  try {
    ...
    for (const table of tables) {
      ...
      // plain JSON’e çevirelim (RowDataPacket → normal object)
      payload.tables[table] = rows.map((r) => ({ ...r }));
    }
 ...
 *  - ?format=sql  (veya boş) → .sql dump (mysqldump / fallback)
 *  - ?format=json           → JSON dump (tüm tablolar)
 * ======================================================================= */
export const adminExportSql: RouteHandler = async (req, reply) => {
  ...
  const baseName = `dump_${stamp.getFullYear()}${pad(
    stamp.getMonth() + 1
  ...
  if ((format || "").toLowerCase() === "json") {
    try {
      const buf = await exportDbAsJsonBuffer();
      const filenameJson = `${baseName}.json`;

      return reply
        .header("Content-Type", "application/json; charset=utf-8")
        .header("Content-Disposition", `attachment; filename="${filenameJson}"`)
        .send(buf);
    } catch (err: any) {
      req.log.error({ err }, "db export json failed");
      return reply.code(500).send({
        error: {
          message: err?.message || "export_json_failed",
        },
      });
    }
  }

  // Varsayılan / SQL export
  const filenameSql = `${baseName}.sql`;
