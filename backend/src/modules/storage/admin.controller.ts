...
// FINAL — Insert typeleri FULL, omitNullish INSERT’te KULLANILMAZ
// Fix:
// - repoInsert(omitNullish(...)) => repoInsert(fullObject)
// - base/recBase explicit StorageInsert type
// - metadata type: Record<string, unknown> | null (schema ile uyumlu)
// =============================================================
...
/** NULL/undefined alanları PATCH/UPDATE için at (INSERT’te kullanma) */
...
export const adminCreateAsset: RouteHandler = async (req, reply) => {
  ...
  const folder = normalizeFolder(s('folder') ?? cfg.defaultFolder ?? null) ?? null;
  ...
  const provider_resource_type = String(up.resource_type || 'image');
  const provider_format = (up.format ? String(up.format) : null) as string | null;
  ...
  // ✅ FULL insert object (required fields guaranteed)
  const base: StorageInsert = {
    id: recId,
  ...
  try {
    // ✅ INSERT’te omitNullish YOK
    await repoInsert(base);
  } catch (e) {
...
export const adminPatchAsset: RouteHandler<{
  ...
  const targetFolder =
    typeof patch.folder !== 'undefined' ? normalizeFolder(patch.folder) : (cur.folder ?? null);
  ...
  if (folderChanged || nameChanged) {
    if (cur.provider_public_id) {
      ...
      sets.provider_version =
        typeof renamed.version === 'number' ? renamed.version : cur.provider_version;
  ...
  // UPDATE tarafında nullish temizliği istersen kullanabilirsin
  await repoUpdateById(req.params.id, omitNullish(sets));
...
/** POST /admin/storage/assets/bulk (multipart mixed) */
export const adminBulkCreateAssets: RouteHandler = async (req, reply) => {
  ...
  let formMeta: Record<string, unknown> | null = null;
  ...
  for await (const part of partsIt) {
    ...
    try {
      ...
      out.push({
        file: cleanName,
        error: { where: 'cloudinary_upload', message: err?.message, http: err?.http_code ?? null },
      });
      continue;
    }
    ...
    try {
      ...
      out.push({
        file: cleanName,
        error: { where: 'db_insert', message: err?.message ?? 'db_insert_failed' },
      });
    }
  }
...
export const adminDiagCloudinary: RouteHandler = async (_req, reply) => {
  ...
  try {
    ...
    return reply
      ...
      .send({
        step: 'api.ping',
        error: { name: err?.name, msg: err?.message, http: err?.http_code },
      });
  }
