...
// FINAL — Custom Pages form (typesafe; aligns with UpsertCustomPageBody)
// Fixes:
// - featured_* fields removed (use image_url/image_asset_id/image_alt)
// - edit mode loads from CustomPageView normalized fields
// - submit body matches UpsertCustomPageBody
// =============================================================
...
import type { UpsertCustomPageBody } from '@/integrations/types';
...
const pickUrlAndId = (res: any) => {
  const url =
    res?.url || res?.public_url || res?.data?.url || res?.asset?.url || res?.file?.url || null;

  const id = res?.asset_id || res?.id || res?.data?.id || res?.asset?.id || res?.file?.id || null;
...
type FormState = {
  ...
  // ✅ Standard image fields (matches types/customPages.ts)
  image_url: string;
...
export default function PageForm() {
  ...
  const { data: pageData, isFetching } = useGetCustomPageAdminByIdQuery(id as string, {
    skip: !isEdit,
  });
  ...
  // Storage upload
  const [uploadAsset, { isLoading: uploadingAny }] = useCreateAssetAdminMutation();
  ...
  // Edit modunda formu doldur (CustomPageView normalize edilmiş alanlar)
  useEffect(() => {
    if (!pageData) return;
    ...
    setFormData((prev) => ({
      ...
      image_asset_id: pageData.image_asset_id ?? '',
  ...
  const normalizedSlugPreview = formData.slug ? slugify(formData.slug) : slugify(formData.title);

  const slugError =
    normalizedSlugPreview.trim().length === 0
      ? 'Slug boş olamaz'
      : existingSlugs.has(normalizedSlugPreview.trim())
  ...
  const previewUrl = '/' + (normalizedSlugPreview || 'sayfa-adi');
  ...
  const uploadToStorage = async (file: File, folder: string, kind: 'cover' | 'content') => {
  ...
  const handleCoverFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { url, id } = await uploadToStorage(file, 'custom_pages/covers', 'cover');
  ...
  const handleQuillImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { url } = await uploadToStorage(file, 'custom_pages/content', 'content');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const normalizedSlug = normalizedSlugPreview;
    ...
    // ✅ FIX: UpsertCustomPageBody uses image_* fields
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      İçerik ve SEO başlıklarında kullanılır.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-muted-foreground">
                        site.com/{normalizedSlugPreview || 'sayfa-adi'}
                      </p>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData((s) => ({ ...s, meta_title: e.target.value }))}
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setFormData((s) => ({ ...s, meta_description: e.target.value }))
                      }
                      rows={2}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData((s) => ({ ...s, image_asset_id: e.target.value }))
                            }
                          />
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData((s) => ({ ...s, image_alt: e.target.value }))
                            }
                          />
                        </div>
                      </div>
                ...
                <div className="space-y-2">
                  ...
                  <ReactQuill
                    ...
                    onChange={(value) => setFormData((s) => ({ ...s, content_html: value }))}
                    className="bg-background"
                    modules={modules}
                  />

                  <p className="text-xs text-muted-foreground">
                    Görseller storage’a yüklenip URL olarak gömülür.
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setFormData((s) => ({ ...s, is_published: checked }))
                      }
                    />
