...
// FINAL — Contact page (createContact + sendEmail)
...
// RTK: contacts + site_settings + functions(send-email)
import {
...
import type { ContactCreateInput, SendEmailBody } from '@/integrations/types';
...
// TR için basit tel: link üretimi
const toTelHref = (raw: string): string => {
  const s = raw.trim();
  if (!s) return '';
  if (s.startsWith('+')) return `tel:${s.replace(/\s+/g, '')}`;
    ...
    try {
      ...
      try {
        if (adminEmailTo) {
          ...
          const mailPayload: SendEmailBody = {
            to: adminEmailTo,
            subject: subjectMail,
            text,
            html,
          ...
          await sendEmail(mailPayload).unwrap();
        }
      } catch (mailErr) {
        // Mail gönderilemese bile form kaydı başarılı → sadece log
      ...
      if (error instanceof z.ZodError) {
        toast.error(error.errors[0]?.message ?? 'Lütfen form alanlarını kontrol edin');
        return;
      }
            ...
            <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a href={telHref} className="text-muted-foreground hover:text-primary">
                        {contactPhoneDisplay}
                      </a>
                    </div>
                  </div>
              ...
              <Card className="gradient-primary text-white">
                <CardContent className="p-6">
                  <h3 className="text-xl font-bold mb-3">WhatsApp ile İletişim</h3>
