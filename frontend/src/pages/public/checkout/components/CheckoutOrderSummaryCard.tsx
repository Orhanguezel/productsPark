...
// FINAL — provider_key as single payment ID (no havale/eft ids)
// =============================================================
...
import { resolveCartItemPricing } from '../checkout.utils';
...
function asRenderableCartItem(x: unknown): CartItemRenderable | null {
  ...
  const quantity = toNum(x.quantity, NaN);
  ...
  const quantity_options = isRecord(productsRaw) ? productsRaw.quantity_options : undefined;

  if (!name) return null;
  if (!Number.isFinite(price) || price < 0) return null;

  return { id, quantity, products: { name, price, quantity_options } };
}
                ...
                {formatPrice(
                  resolveCartItemPricing({
                    ...
                    products: {
                      ...
                      quantity_options: Array.isArray(item.products.quantity_options)
                        ? (item.products.quantity_options as Array<{
                            quantity?: unknown;
                            price?: unknown;
                          }>)
                        : null,
