...
// FINAL — central types compatible (CategoryRow)
// - CategoryRow.slug?: string|null => safe routing
// - CategoryRow has no description/image_url => extend with SubcategoryMeta
// - remove any
// =============================================================
...
type Subcategory = CategoryRow & {
  // UI meta (BE/FE may provide; optional)
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {subcategories.map((subcat) => {
            const slug = typeof subcat.slug === 'string' ? subcat.slug : '';
            const color = (slug && colorMap[slug]) || FALLBACK_GRADIENT;

            const imageUrl = subcat.image_url || FALLBACK_IMAGE;
            ...
            return (
              <Card
                ...
                onClick={() => {
                  if (!slug) return;
                  window.location.href = `/kategoriler/${slug}`;
                }}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
              ...
              >
                <div className="relative overflow-hidden aspect-[4/3]">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-smooth flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <Button size="sm" variant="secondary" type="button" disabled={!slug}>
                ...
                <CardContent className="p-4">
                  <h3 className="font-semibold mb-2 line-clamp-1">{subcat.name}</h3>

                  {description ? (
                    <p className="text-sm text-muted-foreground line-clamp-2">{description}</p>
                  ) : (
                    <p className="text-sm text-muted-foreground line-clamp-2">
                      Bu kategorideki ürünleri inceleyin.
                    </p>
                  )}
                </CardContent>
