// =============================================================
// FILE: src/components/support/TicketManagement.tsx  (UPDATED)
// =============================================================
...
export function TicketManagement() {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      // 3) E-posta bildirimi (RTK functions.sendEmail)
      try {
        // Ticket modelinde email alanı nasıl geldiyse ona göre fallback yapıyoruz
        const anyTicket = selectedTicket as unknown as {
          user_email?: string | null;
          email?: string | null;
        };
        const to =
          anyTicket.user_email ||
          anyTicket.email ||
          null;

        if (to) {
          const subject = `Destek Talebiniz Güncellendi: ${selectedTicket.subject}`;
          const plain = replyMessage.trim();
          ...
          await sendEmail({
            ...
            text:
              `Merhaba,\n\n` +
              `"${selectedTicket.subject}" başlıklı destek talebinize yeni bir yanıt verildi:\n\n` +
              `${plain}\n\n` +
              `İyi günler dileriz.`,
          }).unwrap();
        } else {
          console.warn(
            "Ticket için email adresi bulunamadı, mail gönderilmedi:",
            selectedTicket.id
          );
        }
      } catch (emailErr) {
        console.error("Ticket reply email send error:", emailErr);
        // Sessiz geçiyoruz, ana iş (reply) başarılı zaten
      }
                ...
                <TableCell>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value={STATUS.WAITING_RESPONSE}>
                        {statusText[STATUS.WAITING_RESPONSE]}
                      </SelectItem>
