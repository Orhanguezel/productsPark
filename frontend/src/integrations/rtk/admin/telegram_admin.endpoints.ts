// =============================================================
// FILE: src/integrations/rtk/admin/telegram_admin.endpoints.ts
// FINAL — Telegram RTK (Admin)
// - Backend: /admin/telegram/test | /send | /event
// - FE backward-compat: useSendTelegramNotificationMutation + useTelegramSendTestMutation
// - FIX: exactOptionalPropertyTypes -> chat_id undefined ise hiç ekleme
// =============================================================
  ...
  endpoints: (b) => ({
    ...
    sendTelegramNotification: b.mutation<SimpleSuccessResp, TelegramNotificationBody>({
      query: (body): FetchArgs => {
        ...
        const eventRaw = bdy.event ?? bdy.type ?? 'new_order';
        const event = toTrimmedString(eventRaw) || 'new_order';
