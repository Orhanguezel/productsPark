...
const tryParse = <T>(x: unknown): T => {
  if (typeof x === "string") { try { return JSON.parse(x) as T; } catch { /* keep */ } }
  return x as T;
};
...
export type CartItem = {
  ...
  meta?: Record<string, unknown> | null; // chosen options, etc.
};

export type ApiCartItem = Omit<CartItem, "qty" | "price" | "subtotal" | "meta"> & {
  qty: number | string;
  price: number | string;
...
const normalizeCartItem = (i: ApiCartItem): CartItem => ({
  ...
  meta: i.meta ? (typeof i.meta === "string" ? tryParse<Record<string, unknown>>(i.meta) : i.meta) : null,
});
...
export type Cart = {
  ...
  note: string | null;
  is_locked?: boolean; // if used during checkout
  created_at: string; // ISO
  updated_at: string | null; // ISO
};

export type ApiCart = Omit<Cart, "items" | "subtotal" | "discount_total" | "total_price" | "updated_at" | "user"> & {
  items: string | ApiCartItem[];
  subtotal: number | string;
...
const normalizeCart = (c: ApiCart): Cart => ({
  ...
  user: c.user ? { id: c.user.id, email: c.user.email ?? null, name: c.user.name ?? null } : null,
  items: Array.isArray(c.items) ? c.items.map(normalizeCartItem) : tryParse<ApiCartItem[]>(c.items).map(normalizeCartItem),
...
export type ListParams = {
  ...
  starts_at?: string; ends_at?: string; // created_at range (ISO)
...
export type AddItemBody = { product_id: string; variant_id?: string | null; qty: number; price?: number | null; meta?: Record<string, unknown> | null };
export type UpdateItemBody = { qty?: number; price?: number | null; meta?: Record<string, unknown> | null };
export type UpdateCartBody = { note?: string | null; currency?: string | null; lock?: boolean };
...
export const cartsAdminApi = baseApi.injectEndpoints({
  endpoints: (b) => ({
    listCartsAdmin: b.query<Cart[], ListParams | void>({
      ...
      transformResponse: (res: unknown): Cart[] => Array.isArray(res) ? (res as ApiCart[]).map(normalizeCart) : [],
    ...
    getCartAdminById: b.query<Cart, string>({
      ...
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
    ...
    listCartItemsAdmin: b.query<CartItem[], string>({
      ...
      providesTags: (_r, _e, id) => [{ type: "Carts", id: `ITEMS_${id}` }],
    }),

    addCartItemAdmin: b.mutation<Cart, { id: string; body: AddItemBody }>({
      query: ({ id, body }) => ({ url: `/carts/${id}/items`, method: "POST", body }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.id }, { type: "Carts", id: `ITEMS_${arg.id}` }, { type: "Carts", id: "LIST" }],
    }),

    updateCartItemAdmin: b.mutation<Cart, { id: string; item_id: string; body: UpdateItemBody }>({
      query: ({ id, item_id, body }) => ({ url: `/carts/${id}/items/${item_id}`, method: "PATCH", body }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.id }, { type: "Carts", id: `ITEMS_${arg.id}` }],
    }),

    removeCartItemAdmin: b.mutation<Cart, { id: string; item_id: string }>({
      query: ({ id, item_id }) => ({ url: `/carts/${id}/items/${item_id}`, method: "DELETE" }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.id }, { type: "Carts", id: `ITEMS_${arg.id}` }, { type: "Carts", id: "LIST" }],
    }),
    ...
    clearCartAdmin: b.mutation<Cart, string>({
      ...
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, id) => [{ type: "Carts", id }, { type: "Carts", id: `ITEMS_${id}` }],
    }),

    mergeCartsAdmin: b.mutation<Cart, MergeBody>({
      query: ({ target_id, source_id }) => ({ url: `/carts/${target_id}/merge`, method: "POST", body: { source_id } }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.target_id }, { type: "Carts", id: arg.source_id }, { type: "Carts", id: "LIST" }],
    }),

    applyCouponCartAdmin: b.mutation<Cart, { id: string; body: ApplyCouponBody }>({
      query: ({ id, body }) => ({ url: `/carts/${id}/coupon`, method: "POST", body }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.id }, { type: "Carts", id: "LIST" }],
    }),

    removeCouponCartAdmin: b.mutation<Cart, string>({
      query: (id) => ({ url: `/carts/${id}/coupon`, method: "DELETE" }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, id) => [{ type: "Carts", id }, { type: "Carts", id: "LIST" }],
    }),

    updateCartAdmin: b.mutation<Cart, { id: string; body: UpdateCartBody }>({
      query: ({ id, body }) => ({ url: `/carts/${id}`, method: "PATCH", body }),
      transformResponse: (res: unknown): Cart => normalizeCart(res as ApiCart),
      invalidatesTags: (_r, _e, arg) => [{ type: "Carts", id: arg.id }, { type: "Carts", id: "LIST" }],
    }),
  }),
