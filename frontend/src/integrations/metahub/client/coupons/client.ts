...
export const coupons = {
  async list(params?: Parameters<typeof couponsApi.endpoints.listCoupons.initiate>[0]) {
    try {
      const data = await store3.dispatch(couponsApi.endpoints.listCoupons.initiate(params ?? {})).unwrap();
  ...
  async getByCode(code: string) {
    try {
      const data = await store3.dispatch(couponsApi.endpoints.getCouponByCode.initiate(code)).unwrap();
      return { data: data as Coupon, error: null as null };
    } catch (e) {
      const { message } = normalizeError3(e);
      return { data: null as Coupon | null, error: { message } };
  ...
  async getById(id: string) {
    try {
      const data = await store3.dispatch(couponsApi.endpoints.getCouponById.initiate(id)).unwrap();
      return { data: data as Coupon, error: null as null };
    } catch (e) {
      const { message } = normalizeError3(e);
      return { data: null as Coupon | null, error: { message } };
